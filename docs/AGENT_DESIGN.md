...
2. **Typed intake:** convert Classroom metadata and parsed attachments into a schema-validated `AssignmentSpecification`; retain unresolved questions rather than hallucinating requirements.
3. **Context minimizer:** retain only assignment fields and user-provided material needed for the selected help mode.
4. **Planner:** select a learning mode: clarify, break down, quiz, feedback rubric, or cite sources.
5. **Generator:** return structured output (`summary`, `steps`, `questions`, `citations`, `limitations`).
6. **Integrity / safety check:** ensure it does not claim a submission occurred, fabricate sources, expose secrets, or replace learner authorship.
7. **Human handoff:** extension displays result and offers revision/feedback; no external mutation follows from agent output.
